"""Render the three backend views as a self-contained HTML page + markdown twin.

One section per pass: the six-layer pipeline (a diagram + a card per layer), the
code at the layers built in a non-standard way, and a trace of the core request.
The pipeline diagram renders via Mermaid with a parse-guard (a diagram the model
gets wrong is dropped, not shown as an error). Code excerpts are highlighted.
"""
...
<style>
  :root {{
    ...
    --accent: #4f46e5; --accent-soft: #eef2ff; --good: #16a34a; --stone: #9aa4b2;
  ...
  * {{ box-sizing: border-box; }}
  ...
  .hero .sub {{ font-size: .94rem; color: #d6d8f5; margin: 0 auto; line-height: 1.6; }}
  ...
  .card-top {{ flex-shrink: 0; padding: 14px 18px 12px; border-bottom: 1px solid var(--line);
    background: linear-gradient(180deg, #fafaff, #fff); font-weight: 700; font-size: .95rem; line-height: 1.35; }}
  ...
  pre code {{ padding: 0; font-size: .73rem; line-height: 1.5; }}
...
{intro}
{sections}
    <footer>Read as six layers &middot; {n_route} routes &middot; {n_handler} handlers &middot; {n_service} services &middot; {n_db} models.</footer>
  </main>
</body>
</html>
"""
...
def render_html(name, shared):
    ...
    subtitle = md(welcome) or (
        f"Every request flows through the same six layers. Core endpoint: {md(ep)}"
        if ep else "Every backend request flows through the same six layers.")

    # The six-layer pipeline is the "big picture" — promoted above the sections.
    ...
    pipe_cards = "\n".join(_card(h, b) for h, b in split_cards(shared.get("pipeline_md", "")))
    sections.append(_section("01", "The pipeline", "route · middleware · handler · service · database · response",
                             "pipe", pipe_cards, intro=_intro(shared, "The pipeline")))

    code_cards = "\n".join(_card(h, b) for h, b in split_cards(shared.get("layercode_md", "")))
    sections.append(_section("02", "The code", "only the layers the team built in a non-standard way",
                             "code", code_cards, intro=_intro(shared, "The code")))

    trace_cards = "\n".join(_card(h, b) for h, b in split_cards(shared.get("trace_md", "")))
    sections.append(_section("03", "The trace", "one request, all six layers, and where state changes",
    ...
    return HTML_TEMPLATE.format(
        ...
        intro=hero_diagram,
        sections="\n".join(sections),
        n_route=c.get("route", 0), n_handler=c.get("handler", 0),
        n_service=c.get("service", 0), n_db=c.get("database", 0),
    )


def render_markdown(name, shared):
    parts = [f"# {name}: backend\n"]
    if shared.get("trace_endpoint"):
        parts.append(f"_Core endpoint: {shared['trace_endpoint']}_\n")
    if (shared.get("overview") or {}).get("welcome"):
        parts.append(shared["overview"]["welcome"].strip() + "\n")
    parts.append("## The pipeline\n")
    parts.append(shared.get("pipeline_md", "").strip() + "\n")
    parts.append("## The code\n")
    parts.append(shared.get("layercode_md", "").strip() + "\n")
